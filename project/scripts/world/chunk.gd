## A 16×128×16 section of the voxel world.
## Stores block IDs, generates its own mesh, and maintains references to adjacent chunks
## so face-culling works across chunk borders.
class_name Chunk
extends Node3D

const SIZE := Vector3i(16, 128, 16)

const NEIGHBOUR_PX := 0  ## +X neighbour
const NEIGHBOUR_NX := 1  ## -X neighbour
const NEIGHBOUR_PZ := 2  ## +Z neighbour
const NEIGHBOUR_NZ := 3  ## -Z neighbour
...
## Flat array of block IDs; index = z*SIZE.x*SIZE.y + y*SIZE.x + x.
var blocks: Array[int]
var mesh_instance: MeshInstance3D
## Adjacent chunks in cardinal directions. See [constant NEIGHBOUR_PX] etc.
...
## Adds the mesh instance to the scene tree and applies a vertex-color material.
## Must be called after [method World.register_chunk] adds this chunk as a child.
func prepare() -> void:
...
## Returns the block ID at local coordinates, or [code]-1[/code] if out of bounds.
func get_block(x: int, y: int, z: int) -> int:
	if x >= 0 and y >= 0 and z >= 0 and x < SIZE.x and y < SIZE.y and z < SIZE.z:
		return blocks[(z * SIZE.x * SIZE.y) + (y * SIZE.x) + x]
	return -1

## Sets the block ID at local coordinates. Out-of-bounds writes are silently ignored.
func set_block(x: int, y: int, z: int, id: int) -> void:
	if x >= 0 and y >= 0 and z >= 0 and x < SIZE.x and y < SIZE.y and z < SIZE.z:
		blocks[(z * SIZE.x * SIZE.y) + (y * SIZE.x) + x] = id

## Fills every block in the chunk with [param id].
func fill(id: int) -> void:
	blocks.fill(id)

## Fills a rectangular sub-region [param from]..[param to] (exclusive) with [param id].
...
## Rebuilds the visual mesh and collision shape from the current block data.
## Faces shared with non-air neighbours are culled; faces on chunk borders
## consult [member neighbours] for cross-chunk occlusion.
		...
		for z in SIZE.z:
			for x in SIZE.x:
				...
				for face in faces:
					...
					var brightness: float = FACE_BRIGHTNESS[normal]
